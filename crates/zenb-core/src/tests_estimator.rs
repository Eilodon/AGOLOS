...
fn test_burst_filtering() {
    ...
    assert!(e3.hr_bpm.unwrap() > 60.0 && e3.hr_bpm.unwrap() < 100.0, 
...
fn test_dt_zero_handling() {
    ...
    assert!(e2.hr_bpm.unwrap() > 60.0 && e2.hr_bpm.unwrap() < 70.0,
...
fn test_no_cached_estimate_on_first_burst() {
    ...
    assert_eq!(e1.hr_bpm, Some(60.0), "Should initialize even if dt < threshold");
}
...
fn test_multiple_bursts() {
    ...
    assert!(e5.hr_bpm.unwrap() != 60.0, "Should update after burst window");
}
...
fn test_confidence_calculation_unchanged() {
    ...
    let e1 = est.ingest(&[60.0, 40.0, 6.0], 0);
    assert!(e1.confidence > 0.0, "Should have confidence with all features");
    
    let e2 = est.ingest(&[60.0, 40.0], 1_000_000); // Missing RR
    assert!(e2.confidence < e1.confidence, "Confidence should drop with missing feature");
}
