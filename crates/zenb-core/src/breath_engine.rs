...
impl BreathEngine {
    pub fn new(mode: BreathMode) -> Self {
        let durations = match mode {
            BreathMode::Dynamic(bpm) => {
                let cycle_us = if bpm <= 0.0 { 10_000_000u64 } else { (60_000_000f32 / bpm).round() as u64 };
                ...
                PhaseDurations { inhale_us: inhale, hold_in_us: holdin, exhale_us: exhale, hold_out_us: holdout }
            }
            BreathMode::Fixed(d) => d,
        };
        Self { pm: PhaseMachine::new(durations), mode }
    }
    ...
    pub fn tick(&mut self, dt_us: u64) -> (Vec<String>, u64) {
        ...
        let names = trans.into_iter().map(|p| match p { crate::phase_machine::Phase::Inhale => "Inhale".into(), crate::phase_machine::Phase::HoldIn => "HoldIn".into(), crate::phase_machine::Phase::Exhale => "Exhale".into(), crate::phase_machine::Phase::HoldOut => "HoldOut".into(), }).collect();
        (names, cycles)
    }
    ...
    pub fn set_target_bpm(&mut self, target_bpm: f32) {
        ...
        let cycle_us = if target_bpm <= 0.0 { 10_000_000u64 } else { (60_000_000f32 / target_bpm).round() as u64 };
        let inhale = (cycle_us as f32 * 0.4).round() as u64;
        let holdin = (cycle_us as f32 * 0.05).round() as u64;
        let exhale = (cycle_us as f32 * 0.45).round() as u64;
        let holdout = cycle_us.saturating_sub(inhale + holdin + exhale);
        self.pm = PhaseMachine::new(PhaseDurations { inhale_us: inhale, hold_in_us: holdin, exhale_us: exhale, hold_out_us: holdout });
    }
...
mod tests {
    ...
    fn fixed_pattern_honored() {
        ...
        let d = PhaseDurations { inhale_us: 1_000_000, hold_in_us: 1_000_000, exhale_us: 1_000_000, hold_out_us: 1_000_000 };
        let mut be = BreathEngine::new(BreathMode::Fixed(d));
        // tick 4s in 1s steps and expect at least 1 cycle
        let mut cycles = 0u64;
        for _ in 0..4 { let (_, c) = be.tick(1_000_000); cycles += c; }
        assert!(cycles >= 1);
    }
}
