...
fn test_enhanced_error_messages() {
    ...
    store.append_batch(&sid, &vec![mk_envelope(&sid, 1, 1000)]).unwrap();
    
    // Try invalid sequence
    let result = store.append_batch(&sid, &vec![mk_envelope(&sid, 10, 10000)]);
    
    match result.unwrap_err() {
        StoreError::InvalidSequence { expected, got, session } => {
...
fn test_crypto_error_details() {
    ...
    match result.unwrap_err() {
        StoreError::NotFound(msg) => {
            assert!(msg.contains("session key"), "Error should mention session key");
        }
