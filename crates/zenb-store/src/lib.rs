...
pub enum StoreError {
    ...
    InvalidSequence { expected: u64, got: u64, session: String },
...
impl safety_swarm::TraumaSource for EventStore {
    fn query_trauma(&self, sig_hash: &[u8], now_ts_us: i64) -> Option<safety_swarm::TraumaHit> {
        match EventStore::query_trauma(self, sig_hash, now_ts_us) {
            Ok(Some(hit)) => Some(safety_swarm::TraumaHit {
                sev_eff: hit.sev_eff,
                count: hit.count,
...
impl EventStore {
    ...
    pub fn record_trauma(
        ...
        let (new_sev, new_count, inhibit_until, decay_rate) = match row {
            Some((old_sev, old_count, old_inhibit, old_decay)) => {
                ...
                let dr = if old_decay > 0.0 { old_decay } else { decay_rate_default };
                (sev, cnt, inhibit, dr)
            }
    ...
    pub fn query_trauma(&self, sig_hash: &[u8], now_ts_us: i64) -> Result<Option<TraumaHit>, StoreError> {
    ...
    pub fn load_active_trauma(&self, limit: usize) -> Result<Vec<([u8; 32], TraumaHit)>, StoreError> {
    ...
    pub fn create_session_key(&self, session_id: &SessionId) -> Result<(), StoreError> {
        ...
        let ciphertext = aead.encrypt(XNonce::from_slice(&nonce), &sk.0[..]).map_err(|_| StoreError::Crypto)?;
    ...
    pub fn load_session_key(&self, session_id: &SessionId) -> Result<SessionKey, StoreError> {
        ...
        let (wrapped, nonce) = row.ok_or_else(|| StoreError::NotFound(format!("session key not found for session {:?}", session_id.0)))?;
        let aead = self.xchacha(&self.master_key);
        let pk = aead.decrypt(XNonce::from_slice(&nonce), wrapped.as_slice())
            .map_err(|e| StoreError::CryptoError(format!("failed to decrypt session key: {:?}", e)))?;
        if pk.len() != 32 { 
            return Err(StoreError::CryptoError(format!("invalid session key length: {}", pk.len()))); 
        }
        let mut k = [0u8;32];
    ...
    pub fn delete_session_keys(&self, session_id: &SessionId) -> Result<(), StoreError> {
        self.conn.execute("DELETE FROM session_keys WHERE session_id = ?1", params![&session_id.0 as &[u8]])?;
    ...
    pub fn get_last_seq(&self, session_id: &SessionId) -> Result<u64, StoreError> {
        let r: Option<i64> = self.conn.query_row(
            "SELECT MAX(seq) FROM events WHERE session_id = ?1",
            params![&session_id.0 as &[u8]], |r| r.get(0)
        ).optional()?;
        Ok(r.unwrap_or(0) as u64)
    }

    pub fn append_batch(&self, session_id: &SessionId, envelopes: &[Envelope]) -> Result<(), StoreError> {
        ...
        for (i, env) in envelopes.iter().enumerate() {
            ...
            if env.seq != expected_seq {
                let err_msg = format!("batch sequence gap at index {}: expected {}, got {}", i, expected_seq, env.seq);
                self.log_append_attempt(session_id, attempt_ts_us, seq_start, seq_end, event_count, false, Some(&err_msg))?;
        ...
        if seq_start != db_max_seq + 1 {
            let err_msg = format!("sequence mismatch: expected {}, got {}", db_max_seq + 1, seq_start);
            // Log before rollback
            let _ = tx.execute(
        ...
        for env in envelopes {
            ...
            let payload_plain = serde_json::to_vec(&env.event)
                .map_err(|e| StoreError::CryptoError(format!("event serialization failed: {}", e)))?;
            
            // Build AAD
            let meta_bytes = serde_json::to_vec(&env.meta)
            ...
            let mut nonce = [0u8; 24];
            rand::thread_rng().fill_bytes(&mut nonce);
            let ct = aead.encrypt(XNonce::from_slice(&nonce), aead::Payload { msg: &payload_plain, aad: &aad })
    ...
    fn log_append_attempt(&self, session_id: &SessionId, attempt_ts_us: i64, seq_start: u64, seq_end: u64, event_count: usize, success: bool, error_msg: Option<&str>) -> Result<(), StoreError> {
        self.conn.execute(
    ...
    pub fn read_events(&self, session_id: &SessionId) -> Result<Vec<Envelope>, StoreError> {
        ...
        while let Some(r) = rows.next()? {
            let seq: i64 = r.get(0)?; let ts_us: i64 = r.get(1)?; let _etype: i64 = r.get(2)?; let meta_bytes: Vec<u8> = r.get(3)?; let payload: Vec<u8> = r.get(4)?; let nonce: Vec<u8> = r.get(5)?;
            // compute aad
            let mut hasher = Hasher::new(); hasher.update(&meta_bytes); let meta_hash = hasher.finalize();
            let mut aad = Vec::new(); aad.extend_from_slice(&session_id.0); aad.extend_from_slice(&(seq as u64).to_le_bytes()); aad.extend_from_slice(&(_etype as u16).to_le_bytes()); aad.extend_from_slice(&ts_us.to_le_bytes()); aad.extend_from_slice(meta_hash.as_bytes());
            let plain = aead.decrypt(XNonce::from_slice(&nonce), aead::Payload { msg: &payload, aad: &aad }).map_err(|_| StoreError::CryptoError("decryption failed".into()))?;
            let event: zenb_core::domain::Event = serde_json::from_slice(&plain).map_err(|_| StoreError::CryptoError("event deserialization failed".into()))?;
            let meta = serde_json::from_slice(&meta_bytes).map_err(|_| StoreError::CryptoError("meta deserialization failed".into()))?;
            out.push(Envelope { session_id: session_id.clone(), seq: seq as u64, ts_us, event, meta });
        }
        Ok(out)
    }
